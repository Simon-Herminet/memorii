// ***************************************Recherche en asynchrone pour les Catégories : ***************************************** 
...
document.getElementById("searchInputCat").addEventListener("input", function () {
    ...
    if (searchText.length >= 3) {
        searchCats(searchText);
    }
});

// On recherche en asynchrone
async function searchCats(searchText) {
    const url = 'index.php?controller=category&action=search';
...
function displaySearchResults(results) {
    ...
    if (results.length === 0) {
        ...
        results.forEach(result => {
            ...
            link.href = "index.php?controller=question&action=QbyCat&id=" + result.id_category;
            link.textContent = result.titre_category;
